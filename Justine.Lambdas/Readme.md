...
WORKDIR /src
COPY ["Justine.Lambdas.csproj", "Justine.Lambdas/"]
RUN dotnet restore "Justine.Lambdas/Justine.Lambdas.csproj"

WORKDIR "/src/Justine.Lambdas"
COPY . .
RUN dotnet build "Justine.Lambdas.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Justine.Lambdas.csproj" \
...
Execute unit tests
```
    cd "Justine.Lambdas/test/Justine.Lambdas.Tests"
    dotnet test
```

Deploy application
```
    cd "Justine.Lambdas/src/Justine.Lambdas"
...
### Reverting to not using Annotations: I am not using Annotations
Lambdas using this
